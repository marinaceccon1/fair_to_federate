...
===========================================
This script computes the advantage for CLIENT 0 when federating with CLIENTS 1 through 2
(i.e. in a 3-client federation).
...
This gives us a clear, unambiguous question:
  "What is the advantage for a client with configuration A
   when federating when federating with 2 other clients?""
...
OUTPUT FORMAT: Matches the structure of federated_vs_standalone.csv, extended with
               client_2 through client_2 config columns.
...
Sources read directly (no intermediate CSVs required):
  ...
  Federated  → evaluation_results_3clients/metrics_auc.csv
               evaluation_results_3clients/metrics_soft_tpr.csv  (or metrics_hard_tpr.csv)
               evaluation_results_3clients/metrics_balanced_accuracy.csv
"""
...
FEDERATED_RESULTS_DIR  = Path("evaluation_results_3clients")
STANDALONE_RESULTS_DIR = Path("evaluation_results_standalone_client0")
OUTPUT_CSV             = Path("federation_advantage_client0_3clients.csv")
...
def load_federated(results_dir: Path) -> tuple[pd.DataFrame, str]:
    """
    Load the three main metric CSVs from evaluation_results_3clients/ and merge them
...
def main():
    print("\n" + "=" * 70)
    print("🔍  COMPUTING FEDERATION ADVANTAGE FOR CLIENT 0  (3-CLIENT SETUP)")
    print("=" * 70)
