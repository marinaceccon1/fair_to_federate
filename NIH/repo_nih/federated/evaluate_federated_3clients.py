"""
Model evaluation script for 3-CLIENT federated models.

Analogous to evaluate_federated_5clients.py, but:
  - Targets models with exactly 3 clients
  - Reads from  best_models_3clients/  (filenames contain ``_3clients_``)
  - Saves results to evaluation_results_3clients/
...
RESULTS_DIR    = Path("evaluation_results_3clients")
RESULTS_DIR.mkdir(exist_ok=True)

NEW_MODELS_DIR = Path(os.environ.get("MODELS_DIR", "best_models_3clients"))
...
def load_existing_results():
    ...
    if not results_file.exists():
        print("ℹ️  No existing 3-client results found — starting fresh")
        return [], set()
    try:
        with open(results_file) as f:
            existing = json.load(f)
        evaluated = {Path(r["model_path"]).name for r in existing}
        print(f"✅ Loaded {len(existing)} existing 3-client results")
...
def get_models_to_evaluate(_existing_models: Set[str]) -> List[Path]:
    """Return all 3-client model files (re-evaluates everything)."""
    return sorted(NEW_MODELS_DIR.glob("best_global_model_nih_3clients__*.pt"))


def parse_model_filename(filename: str) -> List[Dict]:
    """
    Parse a filename of the form produced by server_nih_Nclients.py:
        best_global_model_nih_3clients__C0_p0.12_M100F0_flip0.0__C1_...pt
    """
    # Strip the prefix and suffix added by the 3-client server
    config_str  = filename.replace("best_global_model_nih_3clients__", "").replace(".pt", "")
...
def evaluate_model(
    ...
    """Evaluate a single 3-client model and return full metrics dict."""
    print(f"\n{'='*70}")
...
# =============================================================================
# SUMMARY TABLES  — writes to RESULTS_DIR (evaluation_results_3clients/)
# =============================================================================
...
def main():
    print("\n" + "=" * 70)
    print("📊 SETTING UP DATA FOR 3-CLIENT EVALUATION")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("🔍 3-CLIENT MODEL EVALUATION")
    ...
    print(f"\n📋 Found {len(all_models)} model files — filtering to 3-client param_grid...")

    valid_models, skipped_models = [], []
    for model_path in all_models:
        try:
            cfg = parse_model_filename(model_path.name)
            if is_valid_config(cfg):          # enforces num_clients == 3
    ...
    print(f"   Valid 3-client models      : {len(valid_models)}")
    print(f"   Skipped (wrong num/config) : {len(skipped_models)}")

    if not valid_models:
        print("\n✅ No valid 3-client models to evaluate.")
        return
    ...
    if skipped_models:
        ...
        with open(skip_file, "w") as f:
            json.dump(
                [{"model": str(m), "reason": "Not a valid 3-client param_grid config"}
    ...
    print("\n" + "=" * 70)
    print("🏁 3-CLIENT EVALUATION COMPLETE")
