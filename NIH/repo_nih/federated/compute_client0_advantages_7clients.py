...
===========================================
This script computes the advantage for CLIENT 0 when federating with CLIENTS 1 through 6
(i.e. in a 7-client federation).
...
For each federated model with:
  ...
  - Client 5: configuration F
  - Client 6: configuration G
...
This gives us a clear, unambiguous question:
  "What is the advantage for a client with configuration A
   when federating when federating with 6 other clients?""
...
OUTPUT FORMAT: Matches the structure of federated_vs_standalone.csv, extended with
               client_2 through client_6 config columns.
...
Sources read directly (no intermediate CSVs required):
  ...
  Federated  → evaluation_results_7clients/metrics_auc.csv
               evaluation_results_7clients/metrics_soft_tpr.csv  (or metrics_hard_tpr.csv)
               evaluation_results_7clients/metrics_balanced_accuracy.csv
"""
...
FEDERATED_RESULTS_DIR  = Path("evaluation_results_7clients")
STANDALONE_RESULTS_DIR = Path("evaluation_results_standalone_client0")
OUTPUT_CSV             = Path("federation_advantage_client0_7clients.csv")
...
def load_federated(results_dir: Path) -> tuple[pd.DataFrame, str]:
    """
    Load the three main metric CSVs from evaluation_results_7clients/ and merge them
    ...
    potential_keys = [
        ...
        "client_5_portion", "client_5_male_pct", "client_5_female_pct", "client_5_flip_frac",
        "client_6_portion", "client_6_male_pct", "client_6_female_pct", "client_6_flip_frac",
    ]
...
def compute_advantages(fed_df: pd.DataFrame, tpr_source: str, sa_lookup: dict) -> pd.DataFrame:
    ...
    for _, row in fed_df.iterrows():
        ...
        out = {
            ...
            # Client 5 config
            "client_5_portion":    row.get("client_5_portion",    np.nan),
            "client_5_male_pct":   row.get("client_5_male_pct",   np.nan),
            "client_5_female_pct": row.get("client_5_female_pct", np.nan),
            "client_5_flip_frac":  row.get("client_5_flip_frac",  np.nan),

            # Client 6 config
            "client_6_portion":    row.get("client_6_portion",    np.nan),
            "client_6_male_pct":   row.get("client_6_male_pct",   np.nan),
            "client_6_female_pct": row.get("client_6_female_pct", np.nan),
            "client_6_flip_frac":  row.get("client_6_flip_frac",  np.nan),
...
def main():
    print("\n" + "=" * 70)
    print("🔍  COMPUTING FEDERATION ADVANTAGE FOR CLIENT 0  (7-CLIENT SETUP)")
    print("=" * 70)
