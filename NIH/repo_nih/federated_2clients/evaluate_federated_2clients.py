...
================================
Model evaluation script for 2-CLIENT federated models.

Reads models from  best_models_2clients/  (filenames contain ``_2clients__``),
evaluates them, and writes results to  evaluation_results_2clients/.
Other N-client CSVs are never touched.

Metrics computed:
  - AUC (macro, micro, per-pathology, by sex)      → metrics_auc.csv
  - F1 (macro, per-pathology, by sex)              → metrics_f1.csv
  - Hard TPR (macro, micro, per-pathology, by sex) → metrics_hard_tpr.csv
...
---------------------
NIH_DATA_PATH   Path to the NIH chest X-ray image directory (required).
GPU_DEVICE      CUDA device string (default: "cuda").
MODELS_DIR      Override the model input directory
...
from src.data.utils import import_cxp_dfs, import_nih_dfs
...
# =============================================================================
# SKLEARN-BASED METRIC FUNCTIONS
# =============================================================================
...
# =============================================================================
# CONFIG VALIDATION  (enforces 2-client param_grid constraints)
# =============================================================================

# Keep a local copy of the grid constants so this script is self-contained
# (avoids importing param_grid_2clients at module level before sys.path is set)
_PORTIONS = [0.12, 0.06, 0.03]
_GENDER_PROPORTIONS = [
    {"Male": 1.0, "Female": 0.0},
    {"Male": 0.5, "Female": 0.5},
    {"Male": 0.0, "Female": 1.0},
]
_FLIP_FRACTIONS = [0.0, 0.15, 0.30, 0.45]


def is_valid_config(config: List[Dict]) -> bool:
    """Return True only for 2-client models whose configs match the param_grid."""
    if len(config) != 2:
        return False
    for client in config:
        if client["portion"] not in _PORTIONS:
            return False
        gender = client["gender"]
        if not any(
            abs(gender["Male"]   - g["Male"])   < 0.001 and
            abs(gender["Female"] - g["Female"]) < 0.001
            for g in _GENDER_PROPORTIONS
        ):
            return False
        if client["flip_frac"] not in _FLIP_FRACTIONS:
            return False
...
# =============================================================================
device = torch.device(
    os.environ.get("GPU_DEVICE", "cuda") if torch.cuda.is_available() else "cpu"
)
...
if path_image is None:
    raise EnvironmentError(
        ...
        "   Export it before running: export NIH_DATA_PATH=/path/to/nih/images"
    )
...
RESULTS_DIR    = REPO_ROOT / "repo_nih" / "evaluation_results_2clients"
RESULTS_DIR.mkdir(exist_ok=True)

NEW_MODELS_DIR = Path(os.environ.get("MODELS_DIR", str(REPO_ROOT / "repo_nih" / "best_models_2clients")))
...
def load_existing_results():
    ...
    if not results_file.exists():
        print("ℹ️  No existing 2-client results found — starting fresh")
        return [], set()
    try:
        with open(results_file) as f:
            existing = json.load(f)
        evaluated = {Path(r["model_path"]).name for r in existing}
        print(f"✅ Loaded {len(existing)} existing 2-client results")
...
def get_models_to_evaluate(_existing_models: Set[str]) -> List[Path]:
    """Return all 2-client model files (re-evaluates everything)."""
    return sorted(NEW_MODELS_DIR.glob("best_global_model_nih_2clients__*.pt"))


def parse_model_filename(filename: str) -> List[Dict]:
    """Parse filename: best_global_model_nih_2clients__C0_p..._M...F..._flip...__C1_....pt"""
    config_str  = re.sub(r"best_global_model_nih_2clients__", "", filename).replace(".pt", "")
...
# =============================================================================
# DATA SETUP  (module-level — runs once on import/exec)
# =============================================================================
print("\n" + "=" * 70)
print("📊 SETTING UP DATA FOR 2-CLIENT EVALUATION")
print("=" * 70)
...
def evaluate_model(
    ...
    val_df_nih,
    test_loader,
    female_paths_test: List[str],
    female_paths_val: List[str],
    common_transform,
) -> Dict:
    """Evaluate a single 2-client model and return full metrics dict."""
    print(f"\n{'='*70}")
    ...
    avg_f1_val = float(np.mean(list(f1_val.values())))
...
def main():
    print("\n" + "=" * 70)
    print("🔍 2-CLIENT MODEL EVALUATION")
    print(f"   Model directory  : {NEW_MODELS_DIR}")
    ...
    print(f"\n📋 Found {len(all_models)} model files — filtering to 2-client param_grid...")
    ...
    print(f"   Valid 2-client models      : {len(valid_models)}")
    print(f"   Skipped (wrong num/config) : {len(skipped_models)}")

    if not valid_models:
        print("\n✅ No valid 2-client models to evaluate.")
        return
    ...
    for i, (model_path, cfg) in enumerate(valid_models, 1):
        ...
        try:
            result = evaluate_model(
                str(model_path), cfg,
                val_df_nih, test_loader,
                female_paths_test, female_paths_val,
                common_transform,
            )
    ...
    if skipped_models:
        ...
        with open(skip_file, "w") as f:
            json.dump(
                [{"model": str(m), "reason": "Not a valid 2-client param_grid config"}
    ...
    print("\n" + "=" * 70)
    print("🏁 2-CLIENT EVALUATION COMPLETE")
